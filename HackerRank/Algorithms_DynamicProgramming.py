...
def longestCommonSubsequence(a, b):
    ...
    matrix_calc = [[0] * (len(b) + 1) for _ in range(len(a) + 1)]

    for i in range(1, len(a) + 1):
        for j in range(1, len(b) + 1):
            if a[i-1] == b[j-1]:
                matrix_calc[i][j] = matrix_calc[i - 1][j - 1] + 1
            else:
                matrix_calc[i][j] = max(matrix_calc[i][j - 1], matrix_calc[i - 1][j])

    max_len = matrix_calc[-1][-1]
    result = []
    curr = len(a) - 1, len(b) - 1
    while max_len:
        if a[curr[0]] == b[curr[1]]:  # match - belongs to common subseq
            result.append(a[curr[0]])
            max_len -= 1
            curr = (curr[0] - 1, curr[1] - 1)
        else:  # go to the square that has longer subsequence until now
            if matrix_calc[curr[0]][curr[1] + 1] > matrix_calc[curr[0] + 1][curr[1]]:
